...
const HomeSection = () => {
  ...
  return (
    <section className={`relative min-h-[90vh] flex items-center justify-center ${styles.animatedBg} overflow-hidden pt-24`}>
      <div className="max-w-5xl w-full mx-auto flex flex-col md:flex-row items-center justify-between gap-12 px-4">
        {/* Left Card */}
        <div
          ref={cardRef}
          className={`glass-card ${styles.glassCard} p-8 md:p-12 flex-1 min-w-[320px] max-w-xl`}
        >
          <div ref={el => (textRefs.current[0] = el)} className="text-[#ff6b6b] text-lg font-bold mb-2 tracking-wider">HELLO!</div>
          <h1
            ref={el => (textRefs.current[1] = el)}
            className={`text-4xl md:text-5xl font-extrabold mb-2 leading-tight ${styles.neonText}`}
            style={{}}
          >
            <span className={`font-extrabold ${styles.typewriter}`}>Devang here!!<span className={styles.cursor}>&nbsp;</span></span>
          </h1>
          <div
            ref={el => (textRefs.current[2] = el)}
            className={`text-xl md:text-2xl font-semibold mb-4 ${styles.neonSub}`}
          >
          ...
          <div className="flex gap-4 mt-4">
            <a
              ...
              className={`px-7 py-2.5 rounded-full font-semibold text-white ${styles.gradientBtn} shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#00e6fb]`}
            >
              View Work
            </a>
            <a
              ...
              className={`px-7 py-2.5 rounded-full font-semibold text-[#ff6b6b] ${styles.outlineBtn} shadow transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#ff6b6b]`}
            >
              Contact Me
            </a>
          </div>
        ...
        <div className="flex-1 flex justify-center md:justify-end items-center">
          <div className={`${styles.glowRing} neon-glow rounded-2xl p-1 md:p-1.5`}>
            <img
              ref={imgRef}
              src={profileImg}
